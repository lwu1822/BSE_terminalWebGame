...
export const level08: Level = {
  ...
  title: 'Danger zone',
  concept:
    "## Deleting with `rm`\n\nThe `rm` command (**r**e**m**ove) deletes files **permanently**. There is no trash can, no recycle bin, and no undo. Once it's gone, it's gone.\n\n## Removing directories\n\nJust like `cp`, removing a directory requires `-r` (**r**ecursive) because the shell won't delete a folder and all its contents unless you explicitly tell it to.\n\n- `rm file.txt` — deletes one file.\n- `rm -r folder` — deletes a folder and everything inside it.\n\n## Safety first\n\nAlways double-check the path before pressing Enter. A wrong character can mean deleting the wrong file. This is why `rm -r` requires the extra flag — it's a safety speed bump.",
  briefing:
